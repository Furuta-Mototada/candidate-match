...
└─────────────────────────────────────────────────────────────────────────┘
    ...
    ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
    │ 1. スクレイピング │────▶│ 2. データ整形  │────▶│ 3. DB保存    │
    │ (scrape_*.ts)│     │              │     │ (PostgreSQL) │
    └──────────────┘     └──────────────┘     └──────────────┘
                                                      │
                    ┌─────────────────────────────────┘
                    ▼
    ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
    │ 4. PDF解析    │────▶│ 5. ベクトル化 │────▶│ 6. クラスタ  │
    │ (PyPDF2)     │     │(Sentence-BERT)│    │(K-Means/SVD) │
    └──────────────┘     └──────────────┘     └──────────────┘
                                                      │
                    ┌─────────────────────────────────┘
                    ▼
    ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
    │ 7. スコア計算 │────▶│ 8. ベクトル計算│────▶│ 9. Web UI    │
    │(議案スコア)   │     │ (議員ベクトル)│     │ (SvelteKit)  │
    └──────────────┘     └──────────────┘     └──────────────┘
\`\`\`
...
ユーザーベクトル:  [0.8, -0.3, 0.2]
議員Aベクトル:    [0.7, -0.4, 0.3]
議員Bベクトル:    [-0.6, 0.5, -0.1]

                  (0.8×0.7) + (-0.3×-0.4) + (0.2×0.3)
類似度(User, A) = ────────────────────────────────────
                        |user| × |A|
...
Phase 3: 重要度設定
┌──────────────────────────────────────────────────────────────────────┐
│  クラスター「経済・財政政策」の重要度を設定してください:              │
│                                                                      │
│  あなたにとってこの分野はどれくらい重要ですか？                      │
│                                                                      │
│     ★☆☆☆☆  ほとんど重要でない (重み: 1)                           │
│     ★★☆☆☆  あまり重要でない   (重み: 2)                           │
│     ★★★☆☆  普通             (重み: 3) ← デフォルト                │
...
│  加重平均による総合類似度:                                           │
│                                                                      │
│       Σ (importance[c] × similarity[c][m])                          │
│  S = ──────────────────────────────────────                          │
│               Σ importance[c]                                        │
│                                                                      │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  🏆 1位  山田 太郎 （○○党・衆議院）          92.3%            │  │
│  │        各分野: 経済 95% | 外交 88% | 社会保障 91%              │  │
│  ├────────────────────────────────────────────────────────────────┤  │
│  │  🥈 2位  鈴木 花子 （△△党・参議院）          87.1%            │  │
│  │        各分野: 経済 82% | 外交 92% | 社会保障 85%              │  │
│  ├────────────────────────────────────────────────────────────────┤  │
│  │  🥉 3位  佐藤 一郎 （□□党・衆議院）          84.5%            │  │
│  │        各分野: 経済 88% | 外交 79% | 社会保障 86%              │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  [📥 セッションを保存]  [🔄 やり直す]  [📤 結果を共有]               │
└──────────────────────────────────────────────────────────────────────┘
...
解決: member_party の期間を使用して member_group を拡張
   ...
   1. 会派開始日を含む政党を特定
      → 政党開始日を拡張開始日として使用（前の会派終了日が上限）
   
   2. 会派終了日を含む政党を特定
      → 政党終了日を拡張終了日として使用（次の会派開始日が上限）
...
Step 1: PDF取得・テキスト抽出
┌────────────────────────────────────────────────────────────┐
│  参議院ウェブサイト → PDF URL スクレイピング → ダウンロード  │
│                                                            │
│  PyPDF2 を使用してテキスト抽出:                            │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ タイトル: ○○法の一部を改正する法律案                │  │
...
└────────────────────────────────────────────────────────────┘
                              │
                              ▼
Step 2: ベクトル化（Sentence-BERT）
┌────────────────────────────────────────────────────────────┐
│  使用モデル: paraphrase-multilingual-mpnet-base-v2         │
│  ・50以上の言語に対応（日本語含む）                        │
│  ・出力: 768次元のベクトル                                 │
│                                                            │
│  テキスト → [0.234, -0.567, 0.891, ..., 0.123] (768次元)  │
│                                                            │
│  意味的に類似したテキストは近いベクトルになる              │
└────────────────────────────────────────────────────────────┘
                              │
                              ▼
Step 3: クラスタリング
...
│  例:                                                       │
│  ・クラスター0 → "環境・エネルギー"                        │
│    説明: 地球温暖化対策や再生可能エネルギーに関する法案群  │
│  ・クラスター1 → "地方財政・交付税"                        │
│    説明: 地方自治体の財政基盤強化と交付税制度の改正       │
└────────────────────────────────────────────────────────────┘
```
...
Step 1: クラスター内の法案を取得
┌────────────────────────────────────────────────────────────┐
│  クラスター「環境・エネルギー」に含まれる法案: 45件        │
│                                                            │
│  bill_cluster_assignments テーブルから取得                 │
└────────────────────────────────────────────────────────────┘
                              │
                              ▼
Step 2: 投票行列を構築
...
Step 3: 重み付き SVD
┌────────────────────────────────────────────────────────────┐
│  法案の結果に応じた重み付け（可決=1.0, 未了=0.2 など）     │
│                                                            │
│  Ṽ = V × diag(w₁, w₂, ..., wₙ)                             │
│                                                            │
│  中心化後に SVD:                                           │
...
│  議員3: [0.52, -0.18, 0.31]                                │
│                                                            │
│  代表法案: 各次元で因子負荷量が最大の法案                  │
│  ─────────────────────────────────────                     │
│  次元1の代表法案: 「○○法案」(loading: 0.89)              │
│   → この次元は「○○」に関する立場を表す                   │
│                                                            │
│  説明分散比:                                               │
│  ───────────                                               │
│  次元1: 35% | 次元2: 22% | 次元3: 15%                      │
│  → 3次元で全体の72%を説明                                  │
└────────────────────────────────────────────────────────────┘
```
