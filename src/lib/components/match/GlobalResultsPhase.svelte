<script lang="ts">
	...
	let activeTab = $state('overview'); // 'overview' | 'analysis' | 'all-candidates' | 'party-match'
	...
	// ── Party matching derived state ──
	let activePartyScores = $derived.by(() => {
		if (!partyScores) return [];
						...
						<table class="members-table">
							<thead>
								<tr>
									...
									<th
										onclick={() => togglePartySort('score')}
										class="sortable highlight-col text-right"
									>
										総合 {partySortField === 'score'
											? partySortDirection === 'asc'
												? '↑'
												: '↓'
											: ''}
							...
							<tbody>
								{#each filteredParties as partyItem, idx (partyItem.partyId)}
									<tr>
										...
										<td
											class="score-cell highlight-col {getSimilarityColor(partyItem.globalScore)}"
										>
											{formatSimilarity(partyItem.globalScore)}
										</td>
										{#each clusterResults as result (result.clusterLabel)}
